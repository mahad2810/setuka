...
def load_actual_gps(filepath=None):
    """Load actual GPS traces into a DataFrame."""
    if filepath is None:
        filepath = os.path.join(os.path.dirname(__file__), "data", "actual_gps.csv")
    df = pd.read_csv(filepath)
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    return df


def load_planned_routes(filepath=None):
    """Load planned route POI stops."""
    if filepath is None:
        filepath = os.path.join(os.path.dirname(__file__), "data", "planned_routes.csv")
    df = pd.read_csv(filepath)
    return df


def load_planned_paths(filepath=None):
    """Load detailed road geometry for planned routes."""
    if filepath is None:
        filepath = os.path.join(os.path.dirname(__file__), "data", "planned_route_paths.csv")
    df = pd.read_csv(filepath)
    return df


def load_anomaly_log(filepath=None):
    """Load ground-truth anomaly log."""
    if filepath is None:
        filepath = os.path.join(os.path.dirname(__file__), "data", "anomaly_log.csv")
    df = pd.read_csv(filepath)
...
def save_results(results_df, output_dir=None):
